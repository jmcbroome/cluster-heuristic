...
idf = pd.read_csv("full_gisaid.tsv",sep='\t')
...
idf["Region"] = idf.region.apply(lambda x:conversion.get(x,x))
idf['Internal'] = idf.origins.isin(conversion.keys())
...
for r, sdf in idf.groupby('Region'):
    cvc[r] = sdf.introduction_node.nunique()
gdf['CluCount'] = gdf['name'].apply(lambda x:cvc.get(x,0))
...
for i,l in enumerate(reg):
    if l in ["CA",'TX','FL','NY',"MA",'PR','CO','NM','TN','SD','WA']:
        plt.text(x=x[i],y=y[i],s=l)
ax = sns.scatterplot(x=x,y=y,color='midnightblue')
...
t.plot("IL",ax=ax,cax=cax,missing_kwds={"color":"yellow","edgecolor":'blue'},figsize=[20,15],legend=True,legend_kwds={"label": "Log-Fold Enrichment of Introductions"})
